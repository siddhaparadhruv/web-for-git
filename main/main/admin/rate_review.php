...
if(isset($_GET['seen'])){
    ...
    if($frm_data['seen']=='all'){
        $update = "UPDATE `rating_review` SET `seen`=?";
        $values = [1];
        if(update($update,$values,'i')){
            alert('success','Marked all as read');
        }else{
            alert('error','Operation Failed');
        }
    }else{
        $update = "UPDATE `rating_review` SET `seen`=? WHERE `sr_no`=?";
...
if(isset($_GET['del'])){
    ...
    if($frm_data['del']=='all'){
        $delete = "DELETE FROM `rating_review`";
        ...
        $delete = "DELETE FROM `rating_review` WHERE `sr_no`=?";
        $values = [$frm_data['del']];
        if(delete($delete,$values,'i')){
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive-md">
                            ...
                            <table class="table table-hover border">
                                ...
                                <tbody>
                                    <?php
                                        $select = "SELECT rr.*,uc.name AS uname, r.name AS rname FROM `rating_review` rr 
