...
if (isset($_POST['toggle_status'])) {
    ...
    $update = "UPDATE `user_cred` SET `status`=? WHERE `id`=?";
...
if(isset($_POST['remove_user'])){
    ...
    $result = delete("DELETE FROM `user_cred` WHERE `id`=? AND `is_verified`=?",[$frm_data['user_id'],0],'ii');
...
if (isset($_POST['search_user'])) {
    ...
    $result = select($query,["%$frm_data[name]%"],'s');
    $i = 1;
    $path = USERS_IMG_PATH;
